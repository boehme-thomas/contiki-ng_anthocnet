...
PROCESS_THREAD(anthocnettest, ev, data)
{
    ...
    static uip_ipaddr_t cooja_addr_2;
    static uip_ipaddr_t destination_addr_2;
    ...
    uip_ip6addr(&cooja_addr_2, 0x2001, 0xdb8, 0x0, 0x0, 0x206, 0x6, 0x6, 0x6);
    uip_ip6addr(&destination_addr_2, 0x2001, 0xdb8, 0x0, 0x0, 0x210, 0x10, 0x10, 0x10);
    ...
    } else if (uip_ipaddr_cmp(&host_addr, &cooja_addr_2)) {
        simple_udp_register(&udp_conn, UDP_PORT, &destination_addr_2, UDP_PORT, udp_rx_callback);
    } else if (uip_ipaddr_cmp(&host_addr, &destination_addr_2)) {
        simple_udp_register(&udp_conn, UDP_PORT, &cooja_addr_2, UDP_PORT, udp_rx_callback);
    }

    int random_value = rand() % 108000; // random value between 0 and 150
    ...
    if (uip_ipaddr_cmp(&host_addr, &cooja_addr) || uip_ipaddr_cmp(&host_addr, &cooja_addr_2)) {
