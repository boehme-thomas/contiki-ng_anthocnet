...
PROCESS(anthocnettest, "AntHocNet multiple sender run");
...
PROCESS_THREAD(anthocnettest, ev, data)
{
    ...
    static struct etimer back_to_life;
    static struct etimer leave_network_timer;
    ...
    while (1)
    {
        ...
        if (etimer_expired(&wait_timer))
        {
            ...
            etimer_set(&leave_network_timer, 15 * CLOCK_SECOND); // check die after 15 seconds
            break;
        }
    }
    ...
    while (1) {
        ...
        if (etimer_expired(&leave_network_timer) && !dead)
        {
            double die = (double)rand();
            // die with a probability of 10%
            LOG_INFO("Leave network timer expired, check for die...\n");
            if (die < RAND_MAX * 0.1) {
                dead = true;
                etimer_stop(&back_to_life);
                etimer_stop(&send_timer);
                int dead_time = rand() % (60 * CLOCK_SECOND) + 60 * CLOCK_SECOND; // random die time between 60 and 120 seconds
                LOG_INFO("Node will die for %d mseconds\n", dead_time);
                etimer_set(&back_to_life, dead_time);
                NETSTACK_ROUTING.leave_network();
                NETSTACK_RADIO.off();
            }
        }

        if (etimer_expired(&send_timer) && !dead) {
            ...
            if (send < RAND_MAX * 0.1) {
                ...
                do
                {
                    ...
                    int i = id + 0x200;
                    uip_ip6addr(&destination_addr, 0x2001, 0xdb8, 0x0, 0x0, i, id, id, id);
                } while (uip_ipaddr_cmp(&destination_addr, &host_addr));
                ...
                LOG_INFO_(" to ");
                LOG_INFO_6ADDR(&destination_addr);
                LOG_INFO_("\n");
        ...
        if (etimer_expired(&back_to_life) && dead)
        {
            LOG_INFO("Back to Life\n");
            NETSTACK_RADIO.init();
            NETSTACK_ROUTING.init();
            NETSTACK_ROUTING.init(); // to ensure the broadcasting of hello messages
            dead = false;
            etimer_set(&send_timer, 10 * CLOCK_SECOND);
            int check_when_to_die = rand() % (900 * CLOCK_SECOND) + 300 * CLOCK_SECOND; // random die time between 5 and 20 minutes
            etimer_set(&leave_network_timer, check_when_to_die); // check die after 5 to 20 minutes
        }
