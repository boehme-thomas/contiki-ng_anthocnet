...
#include "../../../../../contiki-ng/os/sys/process.h"
...
PROCESS(anthocnettest, "AntHocNet to sink node run");
...
PROCESS_THREAD(anthocnettest, ev, data)
{
    ...
    // set node with id 1 as sink node
    uip_ip6addr(&destination_addr, 0x2001, 0xdb8, 0x0, 0x0, 0x201, 0x1, 0x1, 0x1);
    ...
    etimer_set(&energest_timer, 120 * CLOCK_SECOND); // energest every 2 minutes
    while (1)
    {
        PROCESS_WAIT_EVENT();
